...
def upgrade(open_id, set_leader_master=False):
    ...
    db_mongo.get_table('plat2', 'member').update({'_id': open_id}, {'$set': {'level': 1, 'leader_openid': open_id,
...
def upgrade_leader(member_id, leader_id):
    ...
    member_items = db_mongo.get_table('plat2', 'member').find({'refer_id': member_id},
                                                              {'open_id': 1})
    db_mongo.get_table('plat2', 'member').update({'refer_id': member_id, 'leader_openid': leader_id})
